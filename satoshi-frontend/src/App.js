...
function App() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const body = {
      transaction_id: txInput,
    };
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/transaction_id`,
        body
      );
  ...
  return (
    <div className="min-h-screen bg-grey px-32 py-16">
      ...
      >
        ...
        />
        <button
          className=" bg-green-dark py-3 font-medium px-14 rounded text-white my-5"
          disabled={loading}
        >
          {loading ? (
            <svg
              role="status"
              className="mr-2 w-6 h-6 text-gray-500 animate-spin dark:text-gray-300 fill-white"
              viewBox="0 0 100 101"
              fill="none"
      ...
      {output && (
        <div className="mt-16">
          <p className=" text-2xl">Status: {output.message}</p>
          <div className=" bg-light-green p-4 mt-4 rounded">
            <p className="text-2xl text-green-dark">
              Transaction ID:{" "}
              <a
                href={`https://explorer.bc-2.jp/tx/${output.data.tx_id}`}
                target={"_blank"}
                className="underline"
              >
                {output.data.tx_id}
